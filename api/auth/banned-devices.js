...
const { getBannedDevices, addBannedDevice, removeBannedDevice } = require("../_lib/supabase");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "DELETE") {
      ...
      const { deviceId } = body;
      if (!deviceId) { sendJson(res, 400, { error: "deviceId required." }); return; }
      const data = await removeBannedDevice(String(deviceId).trim());
      sendJson(res, 200, data);
      return;
    }
    ...
    sendJson(res, error.statusCode || 500, { error: error.message || "Could not manage banned devices." });
  }
};
